...
set Q4_all_assets = {'Russell_1000',
					 'Russell_2000_Value',
					 'Russell_2000_Growth',
					 'EAFE_Intl_Index',
					 'Barclays_US_Bonds',
...
var allocation{Q4_all_assets};

impvar portfolio_return=sum {i in Q4_all_assets} allocation[i]*mean_return[i];
impvar portfolio_variance=sum {i in Q4_all_assets, j in Q4_all_assets} allocation[i]*allocation[j]*stdev[i]*stdev[j]*correlation[i,j];
impvar portfolio_stdev = sqrt(portfolio_variance);

num initial_return = 4.25;
num req_return init initial_return;

con allocate_assets: sum{i in Q4_all_assets} allocation[i]=1;
con required_return: portfolio_return = req_return;

con allocation['Russell_1000'] >=0;
con allocation['Russell_2000_Value'] >=0;
con allocation['Russell_2000_Growth'] >=0;
con allocation['EAFE_Intl_Index'] >=0;
con allocation['Barclays_US_Bonds'] >=0;
con allocation['Barclays_IG_Bonds'] >=0;
con allocation['Barclays_HY_Bonds'] >=0;
con allocation['GSCI_Precious_Metals'] >=0;
con allocation['FTSE_REIT_Index'] >=0;
...
num eff_allocation {portfolios,Q4_all_assets};
...
for {p in portfolios} do;
	...
	for {j in Q4_all_assets} eff_allocation[p,j] = allocation[j];
end;
...
create data efficient from [eff_port] return min_var min_stdev linear_approx {j in Q4_all_assets} <col(j)=eff_allocation[eff_port,j]>;
