...
set Q2_all_assets = {'Russell_1000','Russell_2000_Value', 'Russell_2000_Growth','Barclays_US_Bonds','Barclays_IG_Bonds'};
...
var allocation{Q2_all_assets} >= 0; 

impvar portfolio_return=sum {i in Q2_all_assets} allocation[i]*mean_return[i];
impvar portfolio_variance=sum {i in Q2_all_assets, j in Q2_all_assets} allocation[i]*allocation[j]*stdev[i]*stdev[j]*correlation[i,j];
impvar portfolio_stdev = sqrt(portfolio_variance);

num initial_return = 4.25;
num req_return init initial_return;

con allocate_assets: sum{i in Q2_all_assets} allocation[i]=1;
...
set portfolios = 1..28; 
num return {portfolios} = [4.25  4.5   4.75  5     5.25  5.5   5.75 
						   6     6.25  6.5   6.75  7     7.25  7.5 
						   7.75  8     8.25  8.5   8.75  9     9.25 
						   9.5   9.75  10    10.25 10.5  10.75 11]; /* required return values */
num min_var {portfolios};
num min_stdev {portfolios};
num dual_val {portfolios}; 					/* for linear approximation  */
num eff_allocation {portfolios,Q2_all_assets};
...
for {p in portfolios} do;
	...
	for {j in Q2_all_assets} eff_allocation[p,j] = allocation[j];
end;
...
create data efficient from [eff_port] return min_var min_stdev linear_approx {j in Q2_all_assets} <col(j)=eff_allocation[eff_port,j]>;
